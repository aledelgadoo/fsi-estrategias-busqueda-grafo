...
def graph_search(problem, fringe):
    ...
    generated_nodes += 1  # Primer nodo creado

    while fringe:
        node = fringe.pop()
        
        # 2. Visitamos el nodo (Interrogamos si es objetivo) 
        visited_nodes += 1
        if problem.goal_test(node.state):
            # Devolvemos la solución y los contadores
            return node, generated_nodes, visited_nodes, node.path_cost
        
        if node.state not in closed:
            closed[node.state] = True
            
            # 3. Expandimos: aquí se crean los nuevos nodos hijos
            successors = node.expand(problem)
            fringe.extend(successors)
            
            # Sumamos la cantidad de hijos creados al total de generados 
            generated_nodes += len(successors)
            
    return None, generated_nodes, visited_nodes, node.path_cost
